...
Do not put manual `viewTransitionName` on the root DOM node inside
`<ViewTransition>`. React's auto-generated name overrides it.
...
**History navigation differs from link navigation:** verify browser back, forward,
and `router.back()` in the installed framework. Preserve history semantics and
use a fallback transition or no animation when the framework does not expose a
supported transition hook.
