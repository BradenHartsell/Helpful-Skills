...
## WebGPU and node-based post-processing

WebGPU and TSL-based post-processing evolve independently from the mature
`EffectComposer` WebGL path. Treat them as a renderer-backend decision, not a
drop-in replacement. Before using node-based materials or post-processing:

1. verify the installed Three.js version and `three/webgpu` and `three/tsl` exports;
2. verify supported browsers and the product's fallback behavior;
3. keep WebGL composer passes and WebGPU node graphs in their own tested render path;
4. confirm the current official examples and APIs in `sources-and-recency.md`.
